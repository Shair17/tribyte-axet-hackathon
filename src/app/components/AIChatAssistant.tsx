...
import { Send, Sparkles, User, Loader2, FileText, Download, AlertCircle, Link as LinkIcon } from 'lucide-react';
...
const getRolePrompts = (role: string) => {
  const prompts = {
    collaborator: [
      { text: 'Axet Gaia, show my current vacation balance', icon: '📊', category: 'personal' },
      { text: 'Axet Gaia, summarize my vacation consumption this year', icon: '📈', category: 'personal' },
      { text: 'Axet Gaia, when should I use my remaining vacation days?', icon: '💡', category: 'personal' },
      { text: 'Axet Gaia, show my vacation history', icon: '📋', category: 'personal' },
    ],
    leader: [
      { text: 'Axet Gaia, show pending approvals for my team', icon: '⏳', category: 'team' },
      { text: 'Axet Gaia, which team members have low vacation balance?', icon: '⚠️', category: 'team' },
      { text: 'Axet Gaia, summarize my team vacation usage', icon: '📊', category: 'team' },
      { text: 'Axet Gaia, detect inconsistencies in my team records', icon: '🔍', category: 'validation' },
    ],
    administrator: [
      { text: 'Axet Gaia, show system-wide vacation statistics', icon: '📊', category: 'global' },
      { text: 'Axet Gaia, detect data inconsistencies across all users', icon: '🔍', category: 'validation' },
      { text: 'Axet Gaia, generate a Q1 vacation report', icon: '📄', category: 'reports' },
      { text: 'Axet Gaia, show Jira-linked vacation records', icon: '🔗', category: 'integration' },
    ]
  };
...
const getAIResponse = (userMessage: string, role: string): { text: string; hasJiraLink?: boolean } => {
...
Would you like me to help you plan your remaining vacation days?`
  ...
  if (lower.includes('my') && (lower.includes('consumption') || lower.includes('usage'))) {
    return {
...
All records are validated and complete.`
  ...
  // Team queries (Leader)
  if (role === 'leader' && lower.includes('pending') && lower.includes('approval')) {
    return {
...
Would you like me to draft reminder messages?`
  ...
  if (role === 'leader' && lower.includes('team') && (lower.includes('usage') || lower.includes('summary'))) {
    return {
...
**Status:** 2 members need to schedule time off urgently.`
  ...
  // Admin queries
  if (role === 'administrator' && lower.includes('system') && lower.includes('statistics')) {
    return {
...
**System health:** ✅ All metrics within normal range`
  ...
  if (lower.includes('inconsisten') || lower.includes('detect')) {
    const scope = role === 'administrator' ? 'system-wide' : role === 'leader' ? 'your team' : 'your records';
    const count = role === 'administrator' ? 3 : role === 'leader' ? 1 : 0;
    
    return {
      text: `**Data Inconsistency Scan Results (${scope}):**

${count > 0 ? `**Issues detected: ${count}**

${role === 'administrator' ? `1. **David Lee** - Overlapping vacation dates with medical leave (Priority: High)
...
3. **2 employees** - Outdated address information (Priority: Low)
   ...
   role === 'leader' ? `1. **Michael Chen** - Future vacation request overlaps with approved team leave
...
${role === 'administrator' || role === 'leader' ? `3. David Lee - Vacation Mar 15-21
...
${role === 'collaborator' ? `• View your vacation balance and consumption
• Track your vacation history
• Get recommendations on time off planning
• Check status of your requests` :

  role === 'leader' ? `• Review pending approvals for your team
...
export default function AIChatAssistant({ isOpen, onOpenChange }: AIChatAssistantProps) {
  const { user } = useAuth();
      ...
      <div className="space-y-2">
        {parts.map((part, index) => {
          ...
          return part.split('\n').map((line, lineIndex) => {
            ...
            if (line.trim().startsWith('🔗')) {
              return (
                <div key={`${index}-${lineIndex}`} className="flex gap-2 items-center text-blue-600 font-mono text-sm">
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:w-[520px] p-0 flex flex-col">
        {/* Header */}
        <SheetHeader className="px-6 py-4 border-b bg-gradient-to-r from-purple-50 via-blue-50 to-cyan-50">
          <div className="flex items-center gap-3">
            <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-600 flex items-center justify-center shadow-lg">
              <Sparkles className="h-7 w-7 text-white" />
            </div>
            <div className="flex-1">
              <SheetTitle className="text-xl">Axet Gaia</SheetTitle>
              <SheetDescription className="text-xs">
                AI Assistant • Role: {user?.role.charAt(0).toUpperCase()}{user?.role.slice(1)}
              </SheetDescription>
            </div>
        ...
        <ScrollArea ref={scrollAreaRef} className="flex-1 px-6 py-4">
          {messages.length === 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center space-y-6 py-8">
              <div className="h-28 w-28 rounded-2xl bg-gradient-to-br from-purple-100 via-blue-100 to-cyan-100 flex items-center justify-center">
                <Sparkles className="h-14 w-14 text-purple-600" />
              </div>
              
              <div className="text-center space-y-2 max-w-sm">
                <h3 className="font-bold text-xl">Hello, {user?.name?.split(' ')[0] || 'there'}!</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  I'm Axet Gaia, your AI-powered assistant. I can help you analyze data, validate information, generate insights, and answer questions tailored to your role.
                </p>
              </div>
              ...
              <div className="bg-muted/50 rounded-lg p-3 w-full">
                <p className="text-xs text-muted-foreground text-center leading-relaxed">
                  🔒 Responses are based on internal system data with role-based access control. All information is confidential and for internal use only.
                </p>
              </div>
            </div>
          ) : (
            ...
            <div className="space-y-6 pb-4">
              {messages.map((message) => (
                ...
                >
                  ...
                  {/* Message Content */}
                  <div className={`flex-1 ${message.sender === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    <Card className={`max-w-[90%] p-4 ${
                      ...
                      {message.sender === 'ai' && (message.text.includes('report') || message.hasJiraLink) && (
                        <div className="flex gap-2 mt-4 pt-3 border-t">
                          <Button size="sm" variant="outline" className="gap-2 text-xs h-8 bg-white">
              ...
              {isTyping && (
                <div className="flex gap-3">
                  <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-600 flex items-center justify-center shadow-sm">
                    <Sparkles className="h-5 w-5 text-white" />
                  </div>
                  <Card className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Loader2 className="h-4 w-4 animate-spin text-purple-600" />
                      <span className="text-sm text-muted-foreground">Axet Gaia is analyzing...</span>
                    </div>
                  </Card>
                </div>
              )}
              ...
              {hasError && (
                <div className="flex gap-3">
                  ...
                  <Card className="flex-1 p-4 border-red-200 bg-red-50">
                    ...
                    <Button size="sm" onClick={handleRetry} variant="outline" className="border-red-300 hover:bg-red-100">
                      Retry
                    </Button>
                  </Card>
                </div>
