...
export default function LeaderDashboard({ user }: LeaderDashboardProps) {
  ...
  const alerts = [
    {
      type: "warning",
      message: "2 team members have less than 5 vacation days remaining",
      priority: "high",
    },
    {
      type: "info",
      message: "David Lee vacation expiring soon (Must use before Nov 30)",
      priority: "medium",
    },
    {
      type: "success",
      message: "All vacation requests processed on time this month",
      priority: "low",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-cyan-50 border-blue-200">
        <CardHeader>
          ...
          <CardDescription className="text-base">
            {user.role} • Managing {teamMembers.length} team members
          </CardDescription>
        </CardHeader>
      </Card>
              ...
              >
                <div className="flex-1 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Reason: {request.reason} • Submitted{" "}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Overview of vacation days for each team member
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {teamMembers.map((member) => {
              ...
              return (
                <div
                  key={member.id}
                  className="space-y-3 p-4 rounded-lg border hover:bg-muted/50 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-sm font-semibold">
                          {member.remaining} days left
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {member.consumed} / {member.totalDays} used
                        </p>
                      </div>
                      <Badge
                        variant="outline"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Important notifications and AI-detected patterns
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {alerts.map((alert, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`flex items-start gap-3 p-4 rounded-lg border ${colors[alert.type as keyof typeof colors]}`}
                >
